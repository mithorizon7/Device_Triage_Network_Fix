...
function getRiskLevel(score: number): { label: string; colorClass: string; bgClass: string } {
  if (score <= 25) {
    return { label: "Low Risk", colorClass: "text-emerald-600 dark:text-emerald-400", bgClass: "bg-emerald-500" };
  }
  if (score <= 50) {
  ...
  return { label: "Critical", colorClass: "text-red-600 dark:text-red-400", bgClass: "bg-red-500" };
}
...
export function RiskMeter({ subscores, total }: RiskMeterProps) {
  ...
  const riskLevel = useMemo(() => getRiskLevel(clampedTotal), [clampedTotal]);
    ...
    <div className="space-y-4" data-testid="risk-meter">
      ...
      <div className="relative h-3 bg-muted rounded-full overflow-hidden">
        <div
          ...
          aria-valuemin={0}
          aria-valuemax={100}
          aria-label={`Total risk: ${clampedTotal}%`}
        />
