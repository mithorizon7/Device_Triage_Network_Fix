...
export function lintScenario(
  ...
  const unknownNetworkIds = uniqueList(networkIds.filter(id => !allowedZoneSet.has(id as typeof ALLOWED_ZONE_IDS[number])));
  ...
  if (controlsRegistry) {
    ...
    if (controlDefinitions.length > 0) {
      const missingControls = controlDefinitions.filter(control =>
