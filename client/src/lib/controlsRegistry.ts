...
export function getScenarioControlDefinitions(
  ...
  return registry.controls.filter((control) => control.applicableScenarios.includes(scenarioType));
}
...
export function getScenarioControlIds(
  ...
  return new Set(
    getScenarioControlDefinitions(registry, scenarioType).map((control) => control.id)
  );
}
